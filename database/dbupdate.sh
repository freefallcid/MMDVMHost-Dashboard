...
echo Downloading DMR-IDs from DMR-Database at Github
curl 'https://raw.githubusercontent.com/DMR-Database/database-beta/master/userat.csv' 2>/dev/null | sed -e 's/\t//g' | awk -F"," '/,/{gsub(/ /, "", $2); printf "%s\t%s\t%s\n", $1, $2, $3}' | sed -e 's/\(.\) .*/\1/g' > /var/www/html/database/dmrids.dat
